...
const DoctorDashboard = () => {
  ...
  const handleCheckIn = async (id) => {
    try {
      ...
      alert("Không thể thực hiện check-in.");
    }
  };
                    ...
                    <td className="px-10 py-6 text-right">
                       <div className="flex justify-end gap-3 translate-x-4 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
                          {app.status === 'Confirmed' && (
                            <button 
                              onClick={(e) => { e.stopPropagation(); handleCheckIn(app.id); }}
                              className="flex items-center gap-2 bg-orange-500 text-white px-5 py-2.5 rounded-xl text-xs font-black shadow-lg shadow-orange-100 hover:bg-orange-600 active:scale-95 transition-all"
                            >
                          ...
                          {app.status === 'Completed' && (
                            <button 
                              onClick={(e) => { e.stopPropagation(); handleViewPatient(app.patientId); }}
                              className="flex items-center gap-2 bg-white text-gray-400 border border-gray-100 px-5 py-2.5 rounded-xl text-xs font-black hover:bg-gray-50 transition-all"
                            >
                              <FileText size={14} />
                              HỒ SƠ
                            </button>
